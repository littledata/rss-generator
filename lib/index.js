...
function generateXML (data){
    ...
    channel.push({ lastBuildDate:   moment().format('ddd, DD MMM YYYY HH:mm:ss ZZ') });
    ...
    ifTruePush(data.pubDate, channel, { 'pubDate': moment(data.pubDate).format('ddd, DD MMM YYYY HH:mm:ss ZZ') });
    ...
    data.items.forEach(function(item) {
        ...
        ifTruePush(item.guid, item_values, { guid:         [ { _attr: { isPermaLink: !item.guid && !!item.url } }, item.guid ]  });
        ...
        if (item.pubDate) {
            ifTruePush(item.pubDate, item_values, { pubDate: item.pubDate });
        }
        else {
            ifTruePush(item.date, item_values, { pubDate: moment(item.date).format('ddd, DD MMM YYYY HH:mm:ss ZZ') });
        }
